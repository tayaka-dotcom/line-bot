    ...
    try {
      ...
      await client.replyMessage(event.replyToken, {
        ...
        text:
          ...
          `LINE登録のためお名前をフルネームで教えてください（LINE messenger）`
      });
    } catch (e) {
      console.error(e);
    }
...
function handleB(event, client, text, userId) {
        ...
        items: [
          {
            ...
            action: {
              type: 'uri',
              label: '名前を入力する',
              uri: 'https://line-webhook-fragrant-glade-1897-app.fly.dev/index.html'
            }
          }
        ]
      }
  ...
  // ここから下は handleB の他の処理（既存のままでOK）
}


// ===============================
// 名前フォームからの POST を受け取る
// ===============================
app.post('/name', async (req, res) => {
  const { userId, name } = req.body;

  // 名前を保存
  userState[userId] = { step: 'afterName', name };

  // LINE に返信（新規入社 / 既存入社）
  await client.pushMessage(userId, [
    {
      type: 'text',
      text: `${name} さん、ご回答ありがとうございます！\n次に、現在の就業状態を教えてください。`
    },
    {
      type: 'text',
      text: '「新規入社」または「既存入社」を選択してください。',
      quickReply: {
        items: [
          {
            type: 'action',
            action: { type: 'message', label: '新規入社', text: '新規入社' }
          },
          {
            type: 'action',
            action: { type: 'message', label: '既存入社', text: '既存入社' }
          }
        ]
      }
    }
